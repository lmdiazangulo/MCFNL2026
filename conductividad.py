...
class FDTD1D:
    ...
    # ------------------------------------------------------------------
    # API pública para configurar materiales
    # ------------------------------------------------------------------

    def set_conductivity(self, sig_array):
        self.sig = np.asarray(sig_array, dtype=float).copy()
        if self.sig.shape != (self.N,):
            raise ValueError(f"sig_array debe tener shape ({self.N},), se recibió {self.sig.shape}")

    def set_permittivity(self, eps_r_array):
        self.eps_r = np.asarray(eps_r_array, dtype=float).copy()
        if self.eps_r.shape != (self.N,):
            raise ValueError(f"eps_r_array debe tener shape ({self.N},), se recibió {self.eps_r.shape}")
        self.eps = self.eps0 * self.eps_r
    ...
    def load_initial_field(self, e0):
        self.e = np.asarray(e0, dtype=float).copy()
    ...
    def _step(self):
        ...
        # --- Actualización interior de E (con conductividad) ---
        ...
        # --- Actualización de H (sin pérdidas magnéticas) 
